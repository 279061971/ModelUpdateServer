...
namespace gridworld
{
	...
	{
		//模型数据服务中心相关设置
		const int DownLoadLimit = 10000; //中地单次网格下载上限
		const int UpLoadLimit = 1000; //单次网格上传上限，人为设定，为了降低上传出错率
		...
		const string strMatchedDataCenter_Config = "ModelUpdateConfig_TwoMFN.xml"; //地调局更新目录下的更新配置文件名

		//自动更新配置文件相关设置
		const char* strConfigSetting = "setting";
		const char* strConfigSetting_RootURL = "RootURL";
		const char* strConfigSetting_MajorMatchedFieldName = "MajorMatchedFieldName"; //用于控制范围的主属性列，如沉积相
		const char* strConfigSetting_MinorMatchedFieldName = "MinorMatchedFieldName"; //用于控制范围的次属性列，如岩性
		...
		const char* strMsg_UpdateMajor = "\t\t\t正在更新主筛选属性%s的第%d个控制范围：%d,剩余%d个控制范围待更新";
		const char* strMsg_UpdateMinor = "\t\t\t\t正在更新次筛选属性%s的第%d个控制范围：%d,剩余%d个控制范围待更新";
		const char* strMsg_UpdateMinor_NoUpdate = "\t\t\t\t次筛选属性%s的控制范围：%d没有更新的钻孔，已跳过";
		const char* strMsg_UpLoad_Failed = "\t\t\t\t%s 错误：次筛选属性%s的控制范围：%d更新的钻孔无法更新到数据库，本范围的更新将被跳过";
		const char* strMsg_GetVariogram_Failed = "\t\t\t\t警告：次筛选属性%s的控制范围：%d的变差函数获取失败，将使用默认变差进行计算";
		const char* strMsg_EvaluateArea_Failed = "\t\t\t\t错误：次筛选属性%s的控制范围：%d的更新范围评估失败，本范围的更新将被跳过,请检查用于评估的变程是否合理";
		const char* strMsg_EvaluateArea_Success = "\t\t\t\t次筛选属性%s的控制范围：%d的更新范围评估成功，共有%d个待更新范围";
		const char* strMsg_GetInterMethod_Failed = "\t\t\t\t错误：从配置文件中获取次筛选属性%s的控制范围：%d的插值算法失败，本范围的更新将被跳过";
		...
		const char* strMsg_GetFaceCode_Failed = "\t\t\t\t错误：从配置文件中获取属性：%s的离散相失败，本范围的更新将被跳过\n";
		const char* strMsg_GetFaceCode_NULL = "\t\t\t\t警告：属性：%s的离散相数量为0，本范围的更新将被跳过";
		...
		const string strErrorMsg_SelectToLayer_FailedToGetMinorCellFromDataBase = "错误：将样本数据筛选到地层步骤-从数据库下载次筛选属性的网格失败";
		...
		const string strErrorMsg_DownLoadData_MajorMatchedFieldNoData = "错误：数据库中当前范围没有主约束属性的值，且无法从其他LOD获取";
		const string strErrorMsg_DownLoadData_MinorMatchedFieldNoData = "错误：数据库中当前范围没有次约束属性的值，且无法从其他LOD获取";
		...
		public:
			...
			* @Param [in]:  strRootURL                    数据服务中心根路径
			* @Param [in]:  strMajorMatchedFieldName      用于筛选网格的主属性列列名称，一般为地层
			* @Param [in]:  strMinorMatchedFieldName      用于筛选网格的次属性列列名称，一般为岩性
			...
			* @Param [in]:  strMatchedFieldName           用于筛选网格的主属性列列名称，一般为地层
			* @Param [in]:  strMinorMatchedFieldName      用于筛选网格的次属性列列名称，一般为岩性
			...
			* @Param [out]: vecCoverageID                 属性场ID集
			* @Param [out]: vecShapeID                    几何场ID集
			* @Param [out]: vecFeatureID                  要素ID集
			* @Param [out]: vecFeatureclassID             要素类ID集
			* @return:      true-获取成功;false-获取失败
			*/
		...
		private:
			...
			* @Param [in]:  vecMajorIDSet                 主约束属性ID集
			* @Param [in]:  vecMinorIDSet                 次约束属性ID集
